...
export const CARD_INFO: Record<GameCard, CardInfo> = {
  ...
  'Сеть информаторов': {
    ...
    artImage: '/cards/spy.jpg',
    bottomIcon: '💰',
    shortDescription: 'Получаете +1 💰 за каждую проверку при дворе в течение круга.',
    fullDescription: 'Выложите перед собой (стоит 1 ⚡). В течение круга (до наступления вашего следующего хода) каждый раз, когда кто-либо объявляет проверку («НЕ ВЕРЮ!») — вы немедленно получаете +1 💰 из казны за перехват сведений.',
    cost: 0,
    targeted: false
  },
  'Досье': {
  ...
  'Право вето': {
    ...
    artImage: '/cards/heir.jpg',
    bottomIcon: '🚫',
    shortDescription: 'Отменяет эффект роли или интриги (5 карт в колоде).',
    fullDescription: 'Инстант ⚡ (стоит 1 ⚡). Играется непосредственно перед применением эффекта Роли или Интриги. Полностью отменяет этот эффект (печати за спор при этом остаются). В колоде 5 таких карт.',
    cost: 0,
    targeted: false
  },
  'Перенаправление': {
    ...
    fullDescription: 'Инстант ⚡ (1 жетон действия). Играется сразу после объявления целевой атаки (Вор, Шантажист). Меняет цель атаки на другого допустимого игрока.',
    cost: 0,
    targeted: true
  },
  'Ва-банк': {
...
/**
 * Создает полную колоду из 39 карт:
 * - 6 Ролей × 3 копии = 18 карт
 * - 4 Интриги × 2 копии = 8 карт
 * - 5 Инстантов:
 *   - Право вето × 5 копий
