...
/**
 * Пауза между последней галочкой «Готов» и первым ходом.
 *
 * Без неё стол подменяется в тот же кадр, в который игрок нажал кнопку: он ещё
 * смотрит на список готовности, а партия уже идёт. Пауза даёт глазу
 * переключиться и прочитать, что все готовы.
 */
export const TOSS_START_MS = 2000;

/**
 * Разброс, с которым боты подтверждают готовность после приземления монетки.
 *
