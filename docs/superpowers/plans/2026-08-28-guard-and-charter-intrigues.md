...
**Goal:** Ввести в игру две новые интриги — «Стража покоев» (нельзя выбрать целью Вора и Шантажиста) и «Охранная грамота» (нельзя потерять короны и нельзя выбрать целью Шантажиста, но и печати не идут).
...
```ts
    ...
    /* Шантажист крадёт, а не уничтожает: себе он забирает ровно столько,
       ...
       В обычной партии сюда с грамотой не попасть — Задача 7 убирает её
       держателя из списка целей Шантажиста. Ветка остаётся страховкой: она
       описывает, что происходит, если защита всё же оказалась на месте к
       моменту применения эффекта. */
...
Общий знаменатель всех четырёх запретов один: **целью не может быть тот, у кого
эта роль заведомо ничего не отнимет.** Пустая казна, ноль корон, «Стража» и
«Грамота» — частные случаи одного правила, поэтому они и живут в одной функции.
...
> `botReactions.ts` править не нужно. Логика «бояться Шантажиста при 4+ коронах» живёт в ветке реакции цели, а после Задачи 7 объявить Шантажиста против держателя грамоты нельзя вовсе — в этой ветке такая цель не окажется. Проверка на грамоту там была бы мёртвым кодом.
...
- [ ] **Step 3: Проверить живьём в браузере**
...
- при заявке «Вора» и «Шантажиста» место соперника со «Стражей покоев» гаснет и не кликается;
- при заявке «Шантажиста» место соперника с «Охранной грамотой» гаснет, а при заявке «Вора» — остаётся кликабельным;
...
Стража окупается, когда у бота есть что отнимать."
```
...
* **Стоимость:** 1 ⚡. Выкладывается открыто перед собой. Перед выкладкой открывается окно вето.
* **Эффект (бессрочный):** Вы **не можете потерять короны** ничем — ни «Обвинением в измене», ни «Тайным заговором», ни дуэлью, ни слухом. Вас при этом **нельзя выбрать целью «Шантажиста»** — ни при объявлении атаки, ни «Перенаправлением»: роль, которая заведомо ничего не отнимет, цель не выбирает. **Золото грамота не защищает** — «Вор» к вам ходит свободно.
...
- [ ] Соперник с «Охранной грамотой» не кликается при заявке «Шантажиста», но кликается при заявке «Вора»
- [ ] Держатель «Охранной грамоты» не теряет корон от «Обвинения в измене» и «Тайного заговора», но и печатей не получает
