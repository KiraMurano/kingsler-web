...
export const dur = {
   ...
   * и кивок получившей заряд. Зеркалятся в CSS как `--dur-pulse-*`
   * (`styles/tokens.css`) — саму анимацию рисует он, а слой карт по этим же
   * числам снимает класс.
   */
  pulseSpent: 0.5,
  pulseCharge: 0.34,
  pulseDisrupt: 0.42,
  /**
   * Сколько сработавшая интрига стоит на месте, прежде чем уйти в сброс.
   *
   * Длиннее самого удара: карта дёрнулась, брызнула искрами — и этому надо
   * дать долю секунды осесть. Улетая сразу, она уносила собственное
   * срабатывание, и от него оставалась вспышка вдогонку.
   */
  pulseHold: 0.85,
  hold: 0.4,
  trail: 0.18
...
export const tilt = {
   ...
   *
   * Разный для разных инстантов, и это не украшение. «Ва-банк» и «Право вето»
   * ложатся в одну и ту же лунку — вето приходит ровно туда, где лежал
   * Ва-банк, которого оно снимает, — и с общим углом две карты совпадали
   * пиксель в пиксель: на столе это читалось как одна карта, моргнувшая
   * картинкой. Встречные наклоны разводят их сразу, ещё в полёте.
   */
  overlay: 14,
  overlayVeto: -13,
  /**
   * «Перенаправление» в той же лунке, что Ва-банк и вето.
   *
   * Перевод сам оверлей, и вето приходит на его место. Общий угол с Ва-банком
   * (+14) склеил бы две карты в одну, как раньше склеивались Ва-банк и вето.
   * 32° — больше четверной дрожи от обоих соседей.
   */
  overlayRedirect: 32,
  /**
   * Встречное вето — то, которым снимают предыдущее.
   *
   * Ложится накрест: два вето подряд приходят в одну и ту же лунку, и по лицу
   * их не различить — обе карты «Право вето». Общий угол дал бы то же
   * совпадение пиксель в пиксель, ради которого разведены Ва-банк и вето, но
   * уже между двумя вето. Накрест — заодно и читается как «снимаю твоё».
   */
  overlayVetoCounter: 19,
  ...
  duelDefender: 6,
  /**
   * Разброс, с которым карта ложится на стол.
   *
   * Кладут карту рукой, а не ставят по линейке: одинаковый угол у всех карт
   * читается как выложенная сетка, а не как ход живого человека. Мал
   * намеренно — это дрожь руки, а не наклон.
   */
  laidJitter: 2.6,
