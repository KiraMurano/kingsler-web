 ...
 * и появилась:
 *
 *  1. вето и Ва-банк, приходящие в одну лунку, разведены по углу заведомо, а
 *     не случайно;
 *  2. вето поверх выкладки Интриги лежит ровно — оно занимает обычную лунку,
 *     где косой угол читается как перекос;
...
/* «Заведомо, а не случайно»: расхождение больше двойной дрожи, то есть его
   не может съесть никакое сочетание идентификаторов. */
assert.ok(
...
const overPlot = cardLie(placed('c2', { kind: 'overlay', over: 'plot' }, 'Право вето'))!;
assert.ok(
